        ...
        with torch.no_grad():
            for test_x, test_y in test_loader:
                ...
                if r2_best < r2:#保存r2最优的模型
                    loss_best = loss_test
                    torch.save(model.state_dict(), os.path.join(path, 'best_test_model.pth'))
                    MAE_best = MAE
                    r2_best = r2
...
def model_prediction(model,descriptor_train,descriptor_test,y_train_real,y_test_real,parent_dire):
    with torch.no_grad():
        ...
        plt.savefig(os.path.join(parent_dire,'test.png'))
        plt.show()
...
label_train = pd.read_csv(os.path.join(current_dir, 'isobutane_train.csv'), header=0, low_memory=False)
descriptor_train = pd.read_csv(os.path.join(current_dir, 'train_list.csv'), header=0, low_memory=False)
label_train_add = pd.read_csv(os.path.join(current_dir, 'isobutane_train_add.csv'), header=0, low_memory=False)
descriptor_train_add = pd.read_csv(os.path.join(current_dir, 'train_add.csv'), header=0, low_memory=False)
data_out = pd.read_csv(os.path.join(current_dir, 'test_opt.csv'), header=0, low_memory=False)
label_train.columns = ['isobutane']
label_train_add.columns = ['isobutane']
log = []
...
loss_best, MAE_best, r2_best = trainer(model,train_loader,test_loader,optimizer,loss_func,epochs,input_size,mb_size,current_dir)

model_best = NNLess_cost()
model_best.load_state_dict(torch.load(os.path.join(current_dir, 'best_test_model.pth')))
...
loss_train,MAE_train,r2_train,loss_test,MAE_test,r2_test, y_train_pre, y_test_pre, y_train_real, y_test_real = model_prediction(model_best,x_train,x_test,y_train,y_test,current_dir)
...
y_out.to_csv(os.path.join(current_dir, 'y_out.csv'),header=True,index=False)
...
torch.save([y_train_pre, y_test_pre, y_train_real, y_test_real],os.path.join(current_dir, 'y_value.pth'))
log.append(result_out)

torch.save(log,os.path.join(current_dir, 'log.pth'))
