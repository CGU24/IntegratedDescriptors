...
root_dir = 'D:/课题组交流/降低实验成本/4_1_C4H10/6高通量计算补充结果/isobutane/'

label_train = pd.read_csv(root_dir + 'isobutane_train.csv', header=0, low_memory=False)
descriptor_train = pd.read_csv(root_dir + 'train_list.csv', header=0, low_memory=False)
label_train_add = pd.read_csv(root_dir + 'isobutane_train_add.csv', header=0, low_memory=False)
